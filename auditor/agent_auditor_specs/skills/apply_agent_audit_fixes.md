...
Applies audit findings by rewriting the audited agent file, updating existing local skill files first, and creating a new local skill only as a last resort.

## Rules

1. Rewrite the audited agent file so it complies with `common/agent_structure/skill_agent_structure.md`.
2. Update an existing local skill to absorb a responsibility before creating a new local skill under the audited agent's `skills/` folder.
3. Create a new local skill only when no existing skill can own the responsibility after modification, merge, or move.
4. Draft or revise local skills according to `common/skill_structure/skill_skill_structure.md`, using `generator/skill_spec_generator/skill_spec_generator.md` requirements when a new local skill is the last resort.
